 ...
 * You should **not** need a runtime dependency. As long as you don't declare a dependency on "journeymapapi" in your mcmod.info file, your mod should load even if JourneyMap doesn't.
...
2. Add a compile dependency on 'info.journeymapapi:journeymapapi-api:#version'
...
dependencies {
    compile group: "info.journeymapapi", name: "journeymapapi-api", version: project.journeymap_api_version, changing: true
}

```

*Note that the journeymapapi-api.jar is built with deobfuscated code so that it can be used at compile time and when
...
* You can put the `journeymapapi-api-*-examplemod.jar` in your runtime mods directory (usually `/run/mods`)
...
1. Write a class that implements the JourneyMap *[journeymapapi.client.api.IClientPlugin](src/main/java/journeymap/client/api/IClientPlugin.java)* interface (like '[ExampleJourneymapPlugin](src/main/java/example/mod/client/plugin/ExampleJourneymapPlugin.java)')
    - Annotate the class with *[@journeymapapi.client.api.ClientPlugin](src/main/java/journeymap/client/api/ClientPlugin.java)* so that JourneyMap can find and instantiate it
