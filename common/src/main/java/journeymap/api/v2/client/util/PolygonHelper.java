...
public class PolygonHelper
{
    ...
    public static List<MapPolygonWithHoles> classifyAndGroup(@Nonnull final List<MapPolygon> polygons)
    {
        ...
        final List<Tuple2<MapPolygon, Area>> hullAreas = hulls.stream()
                .map(hull -> new Tuple2<>(hull, toArea(hull)))
        ...
        for (final MapPolygon hole : holes)
        {
            ...
            {
                final Area hullArea = hullAreas.get(index).b();
                if (contains(hullArea, holeArea))
                {
                    final long area = ringArea(hullAreas.get(index).a());
